...
type FilterKey = "status" | "priority" | "division";
type SortField = "requester" | "quantity" | "priority" | "createdAt" | "status" | "resolvedBy";
  ...
  const filteredRequests = useMemo(() => {
    let filtered = requests.filter((req) => {
      ...
      if (searchTerm) {
        ...
        if (!matchesRequester && !matchesItem && !matchesDivision && !matchesPriority && !matchesNotes && !matchesSerial && !matchesAssignmentSerial && !matchesClothingSize && !matchesShoeSize) {
          return false;
        }
      }
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6">
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          ...
          <div className="flex gap-2 flex-wrap">
            {closedStatuses.map((status) => {
              ...
              return (
                ...
                >
                  {status === RequestStatus.FULFILLED ? "טופל" : status === RequestStatus.PARTIALLY_FULFILLED ? "טופל חלקית" : status === RequestStatus.DENIED ? "נדחה" : status === RequestStatus.CANCELLED ? "בוטל" : "אחר"} ({count})
                </button>
              );
            })}
          </div>
          ...
          {Object.values(filters).some((f) => f !== "ALL") && (
            <button
              onClick={() => setFilters({ status: "ALL", priority: "ALL", division: "ALL" })}
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-flex px-2 py-1 rounded-full text-xs font-bold border ${
                          ...
                          {req.status === RequestStatus.FULFILLED ? "טופל" : req.status === RequestStatus.PARTIALLY_FULFILLED ? "טופל חלקית" : req.status === RequestStatus.DENIED ? "נדחה" : req.status === RequestStatus.CANCELLED ? "בוטל" : req.status === RequestStatus.IN_PROGRESS ? "בטיפול" : "פתוח"}
                        </span>
                      </td>
                    </tr>
                  );
