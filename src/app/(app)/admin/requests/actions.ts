...
type HandleRequestItemInput = z.infer<typeof HandleRequestItemSchema>;

const RequestItemBatchOpSchema = z.object({
  requestItemId: z.string().min(1),
  action: z.enum(["FULFILL", "DENY", "CANCEL", "RESET"]),
  quantity: z.number().int().positive().optional(),
  serialNumber: z.string().optional(),
...
const HandleRequestItemsBatchSchema = z.object({
  requestId: z.string().min(1),
  operations: z.array(RequestItemBatchOpSchema).min(1).max(50),
});

/** Full request shape for admin detail modal (matches list page + assignment snapshot). */
async function fetchAdminRequestDetailForModal(requestId: string) {
  ...
  const equipmentItemIds = [...new Set(row.items.map((i) => i.equipmentItemId))];
  const assignments =
    equipmentItemIds.length === 0
      ...
      : await prisma.assignment.findMany({
          where: {
            ...
            equipmentItemId: { in: equipmentItemIds },
            active: true,
          },
          select: {
            equipmentItemId: true,
...
async function executeHandleRequestItemInTransaction(
  tx: Prisma.TransactionClient,
  session: { user: { id: string } },
  data: HandleRequestItemInput,
) {
const request = await tx.request.findUnique({
...
const requestItem = request.items.find((item) => item.id === data.requestItemId);
...
if (data.action === "RESET") {
  ...
  if (data.action === "FULFILL") {
    ...
    if (requiresSerialNumber && !data.serialNumber) {
    ...
    const quantityToFulfill = data.quantity ?? requestItem.quantity;
    ...
    if (isDeclaration && markStatus) {
        ...
        if (assignment.quantity <= remaining) {
          await tx.assignment.update({
            ...
            data: { 
              ...
              serialNumber: data.serialNumber || assignment.serialNumber,
      ...
      let serialNumberToSave = data.serialNumber;
      for (const assignment of existingAssignments) {
        if (remaining <= 0) break;
  ...
  await writeAuditLog(tx, {
    ...
    entity: AuditEntity.REQUEST_ITEM,
    entityId: data.requestItemId,
    action: `REQUEST_ITEM_${data.action}ED`,
    beforeJson: { status: requestItem.status, quantity: requestItem.quantity },
    afterJson: { status: newItemStatus, quantityFulfilled: data.action === "FULFILL" ? data.quantity : undefined },
    metadataJson: {
      requestId: data.requestId,
      requestType: request.type,
      requesterId: request.requesterId,
...
export async function handleRequestItemAction(formData: FormData) {
  ...
  await prisma.$transaction(
    async (tx) => {
      await executeHandleRequestItemInTransaction(tx, session, parsed.data);
    },
    {
      maxWait: 10000,
      timeout: 15000,
  ...
  const updatedRequest = await fetchAdminRequestDetailForModal(parsed.data.requestId);
...
export async function handleRequestItemsBatchAction(formData: FormData) {
  ...
  let opsJson: unknown;
  try {
    opsJson = JSON.parse(opsRaw);
  } catch {
    throw new Error("נתונים לא תקינים (JSON).");
  }
  const parsed = HandleRequestItemsBatchSchema.safeParse({
  ...
  await prisma.$transaction(
    async (tx) => {
      for (const op of parsed.data.operations) {
        await executeHandleRequestItemInTransaction(tx, session, {
          requestId: parsed.data.requestId,
          requestItemId: op.requestItemId,
          action: op.action,
  ...
  return { updatedRequest: await fetchAdminRequestDetailForModal(parsed.data.requestId) };
}
